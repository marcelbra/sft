...
def get_arguments(parser: ArgumentParser) -> ArgumentParser:
    ...
    # Hf predictions `predictions_hf.json` are default, pass `predictions_vllm.json` for vllm predictions
    parser.add_argument("--predictions_name", type=str, help="Specifies the name of predictions file in the output directory.", default="predictions_hf.json")
    parser.add_argument("--postprocessed_name", type=str, help="File name of evaluation (by sample).", default="postprocessed.json")
    parser.add_argument("--metrics_name", type=str, help="File name of summarized metrics.", default="model_metrics.json")
...
def filter_intermediate(question: str) -> str:
    return format_number(question.split(">>")[-1].split()[0])

def filter_end_result(question: str) -> str:
    return format_number(question.split("#### ")[1].split("\n")[0])
...
def create_processed_results(args: Namespace) -> int:
    ...
    for i, sample in enumerate(samples):
        ...
        filter = filter_intermediate if args.expert else filter_end_result

        failed = False
        try:
            ground_truth_value = filter(sample["instruction"])
    ...
    failed = (round(fail_counter/len(samples), 4), fail_counter)
...
"""
Command:
sbatch --wrap="python3 sft/evaluate_.py --run_name deepseek-7b-baseline-2 "
"""